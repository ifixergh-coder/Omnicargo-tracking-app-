...
export default function DriverPage() {
  ...
  async function confirmStop() {
    ...
    if (tripId) {
      await supabase.from('trips').update({ ended_at: new Date().toISOString() }).eq('id', tripId)
    }
    setSharing(false)
        ...
        {!sharing ? (
          <>
            ...
            <button
              ...
              className="w-full bg-orange text-white font-medium py-3 rounded-md disabled:opacity-40 mt-2"
            >
          ...
          <>
            <p className="text-sm text-slate mb-4">
              {origin || '—'} → {destination === 'Other' ? customDestination : destination || '—'}
            </p>
        ...
        {showPinPrompt && (
          <div className="mt-4 border-t pt-4">
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-md px-4 py-3 mb-2 text-navy"
            />
            {pinError && <p className="text-xs text-red-600 mb-2">{pinError}</p>}
            <div className="flex gap-2">
              <button onClick={() => setShowPinPrompt(false)} className="flex-1 border border-gray-300 text-slate py-2 rounded-md text-sm">
                Cancel
              </button>

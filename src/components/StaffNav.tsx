...
import { Package, PlusCircle, Truck, ScanLine, LogOut } from 'lucide-react'
  ...
  return (
    <>
          ...
          {LINKS.map(({ to, label, icon: Icon }) => (
            <Link
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                isActive(to) ? 'bg-orange text-white shadow-sm' : 'text-white/70 hover:bg-white/10 hover:text-white'
              }`}
            >
              <Icon size={16} />
              {label}
      ...
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-navy border-t border-white/10 flex items-center justify-around py-2 z-50">
        {LINKS.map(({ to, label, icon: Icon }) => (
          <Link key={to} to={to} className={`flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition-colors ${isActive(to) ? 'text-orange' : 'text-white/60'}`}>

...
export default function StaffNav() {
  ...
  return (
    <>
      <div className="hidden md:flex items-center justify-between bg-navy px-6 py-3 mb-6 flex-wrap gap-y-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-navy border-t border-white/10 flex items-center justify-around py-2 z-50 overflow-x-auto">
        {LINKS.map(({ to, label, icon: Icon }) => (
          <Link key={to} to={to} className={`flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-colors shrink-0 ${isActive(to) ? 'text-orange' : 'text-white/60'}`}>
